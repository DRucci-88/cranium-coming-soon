  ...
  for (let i = startIndex; i <= endIndex; i++) {
    imgItems[i] = (
      <>
        ...
        {/* Modal Box */}
        <div
          className={`${modalBoxOpen ? '' : 'hidden'} fixed w-screen h-screen bg-neutral-900/50 flex justify-center items-center top-0 left-0 z-50`}
          ...
          <div className="h-full sm:h-fit text-center overflow-y-auto px-4 lg:px-0 py-8">
            ...
            <div>
              <Button type="button" href={images[debounceImgIndex].link} newTab={true} variant="primary" size="large"
                      isCustom={true} className="mt-6 mx-auto">Experience Now</Button>
            </div>
          </div>
        </div>
      </>
    ...
    <>
      <div className={`${className ?? ''} w-full h-full`}>
        ...
        >
          ...
          >
            <span
              className="inline-flex justify-center items-center w-8 h-8 rounded-full sm:w-10 sm:h-10 bg-neutral-900/50 hover:bg-neutral-900">
